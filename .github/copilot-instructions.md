...
- **Architecture**: Laravel 13 API + Blade shell + React 19/TypeScript via Vite. Blade routes in [routes/web.php](routes/web.php) return minimal views that mount React roots with `data-*` props (see [resources/views](resources/views)). React entrypoints are configured in [vite.config.ts](vite.config.ts) with alias `@` → `resources/js` and shadcn/Tailwind v4 enabled.
