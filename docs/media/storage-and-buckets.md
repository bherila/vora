...
Uploads use a single presigned PUT (R2 accepts up to ~5 GiB per PUT, which covers
`MEDIA_VIDEO_MAX_BYTES`). This matches the vetvet sibling app — neither uses
multipart uploads.
...
they left behind. A short R2 lifecycle rule on the upload prefix is a useful
backstop for objects whose row is already gone.
